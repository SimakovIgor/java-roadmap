...
![Урок](https://img.shields.io/badge/урок-4%20из%209-3b82f6)
![Уровень](https://img.shields.io/badge/уровень-junior-16a34a)
![Тема](https://img.shields.io/badge/тема-List%20·%20Set%20·%20Map-8b5cf6)
...
<p align="center">
  <img src="assets/collections-hierarchy.svg" width="860" alt="Иерархия коллекций: Collection делится на List, Set, Queue, а Map стоит отдельно как пары ключ-значение">
</p>
...
![](assets/04-collections/image2.png)
...
![](assets/04-collections/image5.png)
...
![](assets/04-collections/image1.png)
...
**Современный способ (Java 10): `var`.**

Начиная с Java 10, компилятор способен сам вывести тип локальной переменной
из правой части выражения, вместо явного типа можно писать `var`:
...
`var` это не «нетипизированная» переменная: тип (`ArrayList<String>`)
по-прежнему фиксируется на этапе компиляции, `var` лишь избавляет от
дублирования очевидного из правой части типа. Особенно полезно для длинных
generic-типов (`Map<String, List<Order>>` и т.п.). Не стоит использовать
`var`, если из правой части не понятно, что за тип получится (например,
`var x = compute();`).
...
![](assets/04-collections/image9.png)
...
### Современная Java: неизменяемые списки `List.of()` (Java 9)

Если список после создания не должен меняться (константный набор значений,
whitelist статусов и т.п.), не обязательно заводить под него `ArrayList`,
подойдёт неизменяемый список:

```java
List<String> statuses = List.of("NEW", "PACKING", "DELIVERING");
// statuses.add("X"); // выбросит UnsupportedOperationException
```

`List.of()` возвращает immutable-коллекцию: любая попытка изменить
её содержимое (`add`/`remove`/`set`) завершится исключением. Это защищает
от случайной мутации там, где список должен быть константой.
...
![](assets/04-collections/image7.png)
...
![](assets/04-collections/image3.png)
...
![](assets/04-collections/image8.png)
...
![](assets/04-collections/image4.png)
...
### Современная Java: `Map.of()` для неизменяемых Map (Java 9)
...
Как и `List.of()`, `Map.of()` создаёт неизменяемую Map, удобно для
справочников/констант, которые не должны меняться после создания.
Ограничение: перегрузки `Map.of()` поддерживают не более 10 пар
«ключ-значение»; для большего количества элементов используйте
`Map.ofEntries(...)`.
...
### Современная Java: `Set.of()` (Java 9)

```java
Set<String> allowedRoles = Set.of("ADMIN", "MANAGER", "SELLER");
```

Неизменяемое множество, как и `List.of()`/`Map.of()`, бросит
`UnsupportedOperationException` при попытке добавить/удалить элемент.
...
### Современный способ: `removeIf()` (Java 8) и Stream

Ручной обход через `Iterator` для удаления по условию по-прежнему
работает, но для большинства случаев это лишний код. С Java 8 у
коллекций есть метод `removeIf(Predicate)`, который делает то же самое
одним вызовом:

```java
List<String> list = new ArrayList<>(List.of("A", "B", "C", "C", "A", "A", "B", "C", "B"));
list.removeIf(str -> str.equals("A"));
System.out.println(list);
// [B, C, C, B, C, B]
```

Если исходную коллекцию менять не нужно (или результат должен попасть в
новый список), удобнее решить задачу через Stream:

```java
List<String> source = List.of("A", "B", "C", "C", "A", "A", "B", "C", "B");
List<String> withoutA = source.stream()
        .filter(str -> !str.equals("A"))
        .toList(); // Stream.toList() (Java 16), неизменяемый список, без Collectors.toList()
```

`removeIf` мутирует существующую коллекцию, нужен, когда важно сохранить
её идентичность (например, ссылка уже передана куда-то ещё). Stream-вариант
создаёт новую коллекцию и ничего не мутирует, ближе к функциональному
стилю, которого мы придерживаемся в проекте.
...
## Современная Java (8/16): record и Comparator.comparing

### Records вместо ручных классов-значений (Java 16)

Класс `Cat` выше, типичный value-object: несколько полей, конструктор,
`toString()`. Такой класс можно свести к одной строке через `record`
(Java 16), компилятор сам сгенерирует конструктор, геттеры-аксессоры
(`name()`, `age()`), а также `equals()`, `hashCode()` и `toString()`:
...
public static void main(String[] args) {
    ...
    System.out.println(cats);
    // [Cat[name=A, age=5], Cat[name=B, age=2], Cat[name=C, age=4]]
}
```

`toString()` записи выглядит иначе, чем в исходном примере
(`Cat[name=A, age=5]` вместо `Cat [A]`), если нужен свой формат, метод
`toString()` можно переопределить прямо в теле `record`, как и любой
другой метод.

### `Comparator.comparingInt` вместо ручного `Comparable` (Java 8)

Вместо того чтобы реализовывать `Comparable` и вручную писать
`if/else`-сравнение по возрасту, порядок сортировки можно задать «на
месте» через `Comparator`, без изменения самого класса:
...
public static void main(String[] args) {
    ...
    cats.sort(Comparator.comparingInt(Cat::age)); // по возрасту, по возрастанию
    System.out.println(cats);

    cats.sort(Comparator.comparingInt(Cat::age).reversed()); // по возрасту, по убыванию
    cats.sort(Comparator.comparing(Cat::name)); // по имени
}
```

Это и есть тот самый совет из блока «Best practice» выше: `Comparator`
позволяет иметь сколько угодно разных способов сортировки одного и того
же класса (по имени, по возрасту, по любому другому полю), не трогая сам
класс и не ограничиваясь единственным `compareTo()`.

**Задание (перепишите на современный синтаксис).** Возьмите класс `Cat`
из примера выше и:

1. Перепишите его как `record Cat(String name, int age) {}`.
2. Уберите реализацию `Comparable` и метод `compareTo()`.
3. Отсортируйте список котов тремя способами, по возрасту по возрастанию,
   по возрасту по убыванию и по имени, используя `Comparator.comparingInt`
   и `Comparator.comparing`, без единого `if`.
...
[← Обобщения (Урок 3)](03-generics.md) · [Оглавление](../README.md) · [Средства ввода-вывода (Урок 5) →](05-io.md)
