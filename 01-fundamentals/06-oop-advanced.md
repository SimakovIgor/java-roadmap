...
![Урок](https://img.shields.io/badge/урок-6%20из%207-3b82f6)
![Уровень](https://img.shields.io/badge/уровень-с%20нуля-16a34a)
![Тема](https://img.shields.io/badge/тема-наследование%20·%20полиморфизм-8b5cf6)

Углублённое изучение объектно-ориентированного программирования: наследование, полиморфизм, переопределение методов

<p align="center">
  <img src="assets/inheritance.svg" width="860" alt="Наследование: класс Animal с методом sound(), потомки Dog и Cat наследуют его и переопределяют по-своему">
</p>
...
>
>[Sealed-классы и интерфейсы (Java 17)](#sealed-классы-и-интерфейсы-java-17)
>
...
![](assets/06-oop-advanced/image1.png)
...
// Animal(a) => Cat(a, b) => BombeyCat(a, b, c)
```
...
одинаковы. Если имя и список аргументов совпадают, а возвращаемый тип
отличается, это не перегрузка, а ошибка компиляции, такие объявления
метода несовместимы. Перегруженным метод считается только тогда, когда
отличается список аргументов (по количеству и/или типам).
...
Начиная с Java 16, не нужно после проверки instanceof отдельно писать
приведение типа ((Cat) animal), можно сразу указать имя переменной
нужного типа прямо в условии. Компилятор сам добавит приведение и даст
доступ к этой переменной внутри ветки, где условие истинно.
...
public class DemoApp {
    public static void main(String[] args) {
        Animal animal = new Cat();
        animal.voice();
        // cat здесь уже имеет тип Cat, отдельный каст (Cat) animal не нужен
...
Что это даёт: меньше шаблонного кода (один явный каст вместо двух
упоминаний типа), меньше риска опечататься в приведении и получить
ClassCastException, компилятор сам следит за тем, что переменная `cat`
доступна только там, где instanceof точно вернул true.

**Задание:** перепишите проверку `if (animal instanceof Cat) { ((Cat) animal).methodFromCatClass(); }`
из примера выше через pattern matching for instanceof.
...
`final` запрещает наследование от класса полностью. Но иногда нужно
наследование разрешить, только не любое, а строго ограниченному и
заранее известному набору подклассов, например, мы точно знаем, что
Animal в нашем приложении может быть только Cat или Dog, и не хотим,
чтобы кто-то в другом файле молча добавил ещё одного наследника. Для
этого в Java 17 появились **sealed**-классы и интерфейсы: в объявлении
класса перечисляется список `permits`, за пределами которого
...
// public class Hamster extends Animal {} // Ошибка компиляции: Hamster не указан в permits
```

Что это даёт:

- список наследников Animal виден прямо в объявлении класса (`permits Cat, Dog`),
  его не нужно искать по всему проекту;
- каждый класс из `permits` обязан быть объявлен как `final`, `sealed` или
  `non-sealed` это заставляет явно решить, можно ли наследоваться от него
  дальше;
- в паре с pattern matching for switch (Java 21) закрытая иерархия
  позволяет компилятору проверить, что в `switch` по типу Animal разобраны
  все возможные варианты, без обязательного `default`.

**Задание:** сделайте иерархию Animal → Cat, Dog из домашнего задания ниже
sealed-классом с `permits Cat, Dog`. Попробуйте добавить третий класс
Hamster с `extends Animal`, не указав его в `permits`, и посмотрите на
ошибку компиляции.
...
[← Введение в ООП (Урок 5)](05-oop-intro.md) · [Оглавление](../README.md) · [Практика ООП (Урок 7) →](07-strings-practice.md)
