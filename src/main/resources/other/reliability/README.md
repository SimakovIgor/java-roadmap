<p align="center">
  <img src="assets/hero.svg" width="860" alt="Надёжность и архитектура: 14 приёмов, которые держат прод">
</p>

# Надёжность и архитектура

![Уроков](https://img.shields.io/badge/уроков-14-3b82f6)
...
Приёмы разбиты на три группы. Стрелка означает «обычно идёт следом»: ретраи не имеют смысла без идемпотентности, ретраи без backoff превращаются в шторм, и так далее

<p align="center">
  <img src="assets/map.svg" width="860" alt="Карта 14 приёмов надёжности: повторы, защита от каскада, надёжная доставка">
</p>
...
| [Claim и lease](14-claim-lease.md) | Несколько инстансов разгребают одну таблицу задач: берут одни и те же строки, блокируют друг друга, держат долгий лок на время внешнего вызова | Фоновые поллеры и воркеры в нескольких инстансах: outbox, рассылки, дозапросы, отложенные задачи |
...
- **Claim и lease** (14) это то, как масштабируется поллер из Outbox (10) и Реконциляции (11): несколько инстансов разбирают таблицу задач через `SKIP LOCKED` и lease по времени, а at-least-once на переклейме снова упирается в идемпотентность (01)
...
- [ ] 14. [Claim и lease](14-claim-lease.md)
