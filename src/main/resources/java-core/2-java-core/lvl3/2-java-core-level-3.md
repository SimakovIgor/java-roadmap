...
![Урок](https://img.shields.io/badge/урок-3%20из%209-3b82f6)
![Уровень](https://img.shields.io/badge/уровень-junior-16a34a)
![Тема](https://img.shields.io/badge/тема-generics%20·%20типобезопасность-8b5cf6)
...
<p align="center">
  <img src="../assets/generics-typesafety.svg" width="860" alt="Generics дают типобезопасность: raw List кидает ClassCastException в рантайме, а List<String> ловит ошибку на компиляции">
</p>
...
## Современная Java: pattern matching for instanceof (Java 16+)

Даже до обобщений код из примера выше можно было сделать безопаснее и короче. До Java 16 `instanceof` только отвечал на вопрос "тот ли это тип", а приведение приходилось делать отдельной строкой. Начиная с Java 16 `instanceof` умеет сразу же связывать значение с переменной нужного типа это называется *pattern matching for instanceof*.

```java
Object obj1 = intBox1.getObj();
Object obj2 = intBox2.getObj();

// старый способ: отдельная проверка + отдельное приведение (см. пример выше)
// современный способ: проверка и приведение, одной строкой
if (obj1 instanceof Integer i1 && obj2 instanceof Integer i2) {
    int sum = i1 + i2; // i1, i2 уже типа Integer, явный каст не нужен
    System.out.println("sum = " + sum);
} else {
    System.out.println("Содержимое коробок отличается по типу");
}
...
Компилятор сам гарантирует: если условие `obj1 instanceof Integer i1` истинно, переменная `i1` внутри `if`-ветки имеет тип `Integer`, и никакого `ClassCastException` возникнуть не может, приведение выполняется под капотом безопасно. Это устраняет ровно ту проблему "проверил, но забыл привести (или привёл не туда)", о которой шла речь выше, но обобщения решают её ещё более системно, поэтому переходим к ним.
...
## Современная Java: `var` и `record` вместо ручных геттеров/сеттеров

**`var` (Java 10)**, когда тип объекта и так очевиден из правой части (`new GenericBox<>(...)`), можно не повторять его слева. Компилятор всё равно выводит точный тип (`GenericBox<String>`, `GenericBox<Integer>`) это просто вывод типа на этапе компиляции, а не "нетипизированная" переменная вроде `var` в JS:

```java
var genStr = new GenericBox<>("Hello");   // выведется GenericBox<String>
var genInt = new GenericBox<>(140);       // выведется GenericBox<Integer>
genStr.showType();
```

**`record` (Java 16)**, если обобщённому классу нужны только хранение значения и геттер (как у `GenericBox`), не нужно вручную писать конструктор, геттер, `equals`/`hashCode`/`toString`. `record` генерирует всё это по объявлению полей:

```java
// компактная неизменяемая generic-обёртка: конструктор, value(), equals/hashCode/toString, из коробки
public record Box<T>(T value) {}
```

```java
var box = new Box<>("Hello");
System.out.println(box.value());  // Hello, геттер называется как поле, без get-
System.out.println(box);          // Box[value=Hello], toString сгенерирован автоматически
```

Поле `value` в `record` неизменяемо (нет сеттера), если нужно "изменить" содержимое коробки, создаём новый `Box<>(newValue)` вместо `setObj(...)` из примера `GenericBox`.

**Задание:** перепишите класс `GenericBox<T>` из примера выше в `record Box<T>(T value)` и уберите класс `GenericsDemoApp`, заменив вызовы `getObj()`/`setObj()` на `value()` и создание нового `Box`.
...
## Современная Java: `@SafeVarargs` для generic varargs

Конструктор `BoxWithNumbers(T... nums)` под капотом создаёт массив `T[]`, а массивы с параметризованным типом небезопасны (JVM не может проверить их содержимое в рантайме из-за type erasure, см. раздел "Ограничения" ниже). Компилятор поэтому выдаёт unchecked-предупреждение о "heap pollution". Если реализация действительно не делает с этим массивом ничего опасного (не публикует его наружу, не смешивает с чужими типами), предупреждение подавляют аннотацией `@SafeVarargs`, она документирует, что безопасность проверена вручную, а не просто заглушена:

```java
public class BoxWithNumbers<T extends Number> {
    private final T[] nums;

    @SafeVarargs // безопасно: массив только читаем внутри класса, наружу не отдаём
    public BoxWithNumbers(T... nums) {
        this.nums = nums;
    }
    // ...
...
`@SafeVarargs` можно поставить только на `static`, `final`, `private` метод или конструктор, то есть там, где переопределение в подклассе исключено (иначе безопасность, которую подтверждает аннотация, было бы легко нарушить).
...
## Ограниченные wildcard'ы и принцип PECS

`<?>` из примера выше это *неограниченный* wildcard: подходит любой тип, но с объектом можно только читать (в него нельзя ничего положить, кроме `null`). Часто нужнее *ограниченные* wildcard'ы, `? extends Тип` и `? super Тип`:

```java
public class BoxWithNumbers<T extends Number> {
    // ...

    // ? extends Number, источник данных (Producer): можно читать любые подтипы Number,
    // но нельзя добавлять элементы (компилятор не знает точный подтип)
    public static double sumAll(java.util.List<? extends Number> nums) {
        double sum = 0.0;
        for (Number n : nums) {
            sum += n.doubleValue();
        }
    ...
    // ? super Integer, приёмник данных (Consumer): можно класть Integer и его подтипы,
    // но читать можно только как Object
    public static void addSampleInts(java.util.List<? super Integer> destination) {
        destination.add(1);
        destination.add(2);
    }
}
...
Правило запоминается как **PECS**, *Producer Extends, Consumer Super*: если из коллекции только читаем (она "производит" для нас данные), используем `? extends T`; если только пишем (она "потребляет" наши данные), `? super T`. Если и читаем, и пишем, обычный `T` без wildcard'а.

# Ограничения

Все ограничения ниже, следствие одного механизма: **type erasure** (стирание типов). Компилятор проверяет типобезопасность обобщений только на этапе компиляции, а в скомпилированном байткоде параметр `T` стирается, заменяется на `Object` (или на верхнюю границу, если она задана через `extends`). В рантайме не существует отдельных классов `GenericBox<String>` и `GenericBox<Integer>`, есть только один класс `GenericBox`, и вызов `obj.getClass()` для обоих вернёт одно и то же. Именно поэтому статические члены не могут ссылаться на `T` (статика общая для всех параметризаций) и обобщённые исключения запрещены (JVM должна различать типы исключений в рантайме, а `T` к этому моменту уже стёрт).
...
[← Обработка исключений (Урок 2)](../lvl2/2-java-core-level-2.md) · [Оглавление](../../../../../../README.md) · [Виды коллекций (Урок 4) →](../lvl4/2-java-core-level-4.md)
