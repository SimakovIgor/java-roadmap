...
![Урок](https://img.shields.io/badge/урок-7%20из%207-3b82f6)
![Уровень](https://img.shields.io/badge/уровень-с%20нуля-16a34a)
![Тема](https://img.shields.io/badge/тема-практика%20ООП%20·%20строки-8b5cf6)
...
<p align="center">
  <img src="../assets/string-immutable.svg" width="860" alt="String неизменяемый: каждый += создаёт новый объект. StringBuilder изменяемый: один объект меняется на месте через append">
</p>
...
ней. Если кодировка не указана, будет использован charset по умолчанию
платформы (`Charset.defaultCharset()`), а не ASCII.

**Современная Java (JEP 400, Java 18):** начиная с Java 18 UTF-8 стал
кодировкой по умолчанию для всей платформы (файлы, потоки,
`new String(byte[])` без явного `Charset`), что делает поведение
предсказуемым на любой ОС. До этого `Charset.defaultCharset()` мог
отличаться в зависимости от локали операционной системы, и один и тот же
код мог давать разный результат на Windows и Linux. Явно указывать
`StandardCharsets.UTF_8`, как в примере с `s6`, по-прежнему хорошая
практика, если код должен одинаково работать и на более старых версиях
Java.
...
### Современная Java: `var` для локальных переменных

Начиная с Java 10 можно не дублировать тип переменной, если он и так
очевиден из инициализатора, компилятор выводит тип самостоятельно (это
**не** динамическая типизация, тип фиксируется на этапе компиляции и
дальше не может измениться).
...
`var` уместен, когда тип и так понятен из правой части (`"Hello "`,
`new Plate(100)`), и неуместен, когда явный тип делает код понятнее
(например, для `Optional<List<String>>` явный тип лучше оставить).

**Задание:** перепишите последний пример из раздела «Конкатенация строк»
(`String str = "Десять: " + (5 + 5);`), объявив переменную через `var`.
...
| `boolean equalsIgnoreCase(String anotherString)` | Сравнение строк без учета регистра                                                                                                                                                                       |
...
### Современная Java (11+): удобные методы String

Java 11–12 добавили в `String` методы, которые раньше приходилось писать
руками через `trim()`/циклы/`StringBuilder`:

```java
public class Main {
    public static void main(String[] args) {
        String raw = "   ";
        System.out.println(raw.isBlank());      // true - строка пустая или состоит из пробелов
        System.out.println("  Java  ".strip()); // "Java" - как trim(), но с учётом Unicode-пробелов
        System.out.println("ab".repeat(3));      // "ababab"

        String multiline = "первая\nвторая\nтретья";
        multiline.lines().forEach(System.out::println); // построчный Stream<String>

        String joined = String.join(", ", "Java", "Kotlin", "Scala");
        System.out.println(joined); // "Java, Kotlin, Scala"

        int value = 200;
        System.out.println("Значение: %d".formatted(value)); // аналог String.format("Значение: %d", value)
    }
}
```

`strip()` предпочтительнее `trim()` в коде, ориентированном на Unicode
(`trim()` понимает только символы `<= U+0020`, `strip()` учитывает
Unicode-пробелы). `formatted(...)` делает то же самое, что и
`String.format(...)`, но вызывается на самой строке, что удобно при
цепочках вызовов.
...
Раньше многострочный текст (например, SQL-запрос или JSON) приходилось
склеивать через `+` и `\n`, из-за чего терялась читаемость и было легко
забыть перенос строки. С Java 15 появились **text blocks**,
...
Отступ, общий для всех строк блока, компилятор автоматически убирает
(он ориентируется на отступ закрывающих `"""`), поэтому текст можно
оформлять с отступами внутри метода, не «портя» содержимое строки.
Внутри text block можно использовать и подстановку через `formatted(...)`.
...
**Современная Java:** в `main()` тип объекта и так очевиден из правой
части (`new Cat(...)`, `new Plate(...)`), поэтому здесь уместен `var`,
как уже обсуждалось в разделе «Конкатенация строк»:

```java
public class MainClass {
    public static void main(String[] args) {
        var cat = new Cat("Barsik", 5);
...
7. *(современный синтаксис)* Переписать объявления локальных переменных в `main()` через `var` там, где тип и так очевиден, а вывод информации о тарелке (`info()`) сделать через `"...".formatted(...)` вместо конкатенации строкой `+`.
...
[← Продвинутое ООП (Урок 6)](../lvl6/1-java-fundamentals-level-6.md) · [Оглавление](../../../../../../README.md) · [Java Core: ООП →](../../2-java-core/lvl1/2-java-core-level-1.md)
