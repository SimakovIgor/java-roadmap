...
![Урок](https://img.shields.io/badge/урок-5%20из%207-3b82f6)
![Уровень](https://img.shields.io/badge/уровень-с%20нуля-16a34a)
![Тема](https://img.shields.io/badge/тема-классы%20·%20объекты%20·%20конструкторы-8b5cf6)

Введение в объектно-ориентированное программирование, классы, объекты, конструкторы, инкапсуляция, модификаторы доступа

<p align="center">
  <img src="../assets/class-object.svg" width="860" alt="Класс это шаблон с полями и методами, объекты это экземпляры класса, у каждого свои значения полей">
</p>

>
> [Что такое класс](#что-такое-класс)
>
>> [Современная Java: toString(), equals() и hashCode() вместо info()](#современная-java-tostring-equals-и-hashcode-вместо-самодельного-info)
>
...
>
>[Современная Java (17/21): record для классов-данных](#современная-java-1721-record-для-классов-данных)
>
...
## Современная Java: toString(), equals() и hashCode() вместо самодельного info()

Метод `info()` в примере выше это самодельная замена методу `toString()`,
который есть у **любого** объекта в Java (он унаследован от класса
`Object`). Правильнее переопределить именно `toString()`: тогда объект
будет корректно печататься везде, где ожидается строковое представление
(`System.out.println(user)`, конкатенация со строкой, логирование), а не
только через специально вызванный метод `info()`.
...
public class User {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof User other)) { // pattern matching for instanceof, Java 16+
            return false;
        }
        return id == other.id;
    }
...
Теперь достаточно `System.out.println(user)`, Java сама вызовет
`toString()`, отдельный метод `info()` больше не нужен. А переопределённые
`equals()`/`hashCode()` дают корректное сравнение пользователей и
позволяют безопасно класть их в `HashSet`/`HashMap`: без переопределения
`equals()` унаследован от `Object` и сравнивает только ссылки, то есть два
разных объекта с одинаковыми полями считались бы разными.
...
**Современный способ (var, Java 10+):** компилятор способен сам вывести
тип локальной переменной из правой части выражения, поэтому вместо
`Cat cat1 = new Cat();` можно написать:

```java
var cat1 = new Cat(); // тип Cat выводится автоматически из new Cat()
```

`var` работает только для локальных переменных (не для полей класса и не
для параметров методов) и не делает Java нестрого типизированным,
тип фиксируется на этапе компиляции, просто не пишется явно.
...
помощью сеттера не получится указать коту отрицательный
возраст.
...
# Современная Java (17/21): record для классов-данных

Классы Cat и User в этом уроке это типичные «классы-данные» (data classes):
в них почти нет поведения, только поля + конструктор + геттеры/сеттеры +
`toString`/`equals`/`hashCode`. Если объекту не нужно меняться после
создания (нет сеттеров, только чтение), начиная с Java 16 для такого
случая есть специальная конструкция, `record`.

```java
public record Cat(String name, String color, int age) {
}
```

Эта одна строка полностью эквивалентна классу с:

- `private final` полями `name`, `color`, `age`;
- публичным конструктором `Cat(String name, String color, int age)`;
- геттерами без префикса `get`: `name()`, `color()`, `age()`;
- корректно переопределёнными `equals()`, `hashCode()` (сравнение по всем
  полям) и `toString()` (вида `Cat[name=Барсик, color=Белый, age=4]`).

```java
var cat1 = new Cat("Барсик", "Белый", 4);
System.out.println(cat1);        // Cat[name=Барсик, color=Белый, age=4]
System.out.println(cat1.name()); // Барсик
```

**Важно!** `record` это неизменяемый (immutable) объект: сеттеров у
него нет, и после создания значения полей изменить нельзя. Если объекту
по смыслу нужно менять состояние (как коту, у которого может меняться
`age`, или пользователю, которому меняют `position` через
`changePosition()`), обычный класс с сеттерами/бизнес-методами по-прежнему
уместен, `record` подходит именно для неизменяемых DTO/value-object, а
не для замены любого класса.

**Задание:** перепишите класс "Сотрудник" из домашнего задания в виде
`record` для случая, когда после создания сотрудника ни одно из полей не
должно меняться, и убедитесь, что `System.out.println(person)` уже даёт
читаемое представление объекта без написания собственного метода
`info()`.
...
[← Крестики-нолики (Урок 4)](../lvl4/1-java-fundamentals-level-4.md) · [Оглавление](../../../../../../README.md) · [Продвинутое ООП (Урок 6) →](../lvl6/1-java-fundamentals-level-6.md)
